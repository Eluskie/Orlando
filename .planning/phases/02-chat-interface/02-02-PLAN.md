...
<objective>
Build the chat UI components with streaming message display and Claude-like minimal styling

Purpose: Enable users to have a conversation with streaming responses. This is the core chat experience that will be used for brand creation in Plan 03.
...
<task type="auto">
  <name>Task 2: Wire chat UI to API with useChat</name>
  <files>
    src/app/(chat)/[brandId]/page.tsx
    src/app/(chat)/page.tsx
  </files>
  <action>
Connect the chat components to the API using useChat hook:

1. Update `src/app/(chat)/[brandId]/page.tsx`:
   - 'use client' directive
   - Import useChat from '@ai-sdk/react'
   - Import DefaultChatTransport from 'ai'
   - Import chat components: ChatMessages, ChatInput
   ...
   - Center content with max-w-3xl mx-auto for readability

2. Update `src/app/(chat)/page.tsx`:
   - This is the "no brand selected" state
   - Keep the welcome message but add a note: "Send a message to start creating your first brand"
   - Add a simple input at the bottom that redirects to a new conversation
   - OR: Make this page also use useChat with a temporary/new conversation flow

   Decision: For simplicity, create a "start chat" experience:
...
5. Auto-scroll works on new messages
  ...
  <done>
Chat interface fully functional. User can send messages and receive streaming responses. Typing indicator shows during streaming. Messages render with markdown support.
  </done>
</task>
