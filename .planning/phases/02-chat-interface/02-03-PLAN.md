...
<task type="auto">
  <name>Task 2: Implement brand creation flow in chat</name>
  <files>
    src/components/chat/brand-card.tsx
    src/components/chat/chat-messages.tsx
    src/app/(chat)/page.tsx
...
4. Update `src/app/(chat)/page.tsx` (new chat / no brand):
   ...
   - Handle onCreateBrand callback:
     ...
     * Update brand store with new brand
...
6. Navigate to /[brandId], sidebar updates with new brand
  </verify>
  <done>
Brand creation works through chat. AI guides user to provide brand name. Confirmation card appears. Creating brand navigates to brand chat.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
Complete the persistence layer and sidebar functionality:
...
2. Update `src/components/chat/chat-sidebar.tsx`:
   - On mount, fetch brands from /api/brands
   - Update brand store with fetched brands
   - Add "Clear Chat" option in brand context menu (right-click or three dots)
   - Clear chat: DELETE /api/conversations/[id]/messages (create this endpoint)
   - Show brand's first message preview or "No messages yet" in sidebar item
   - Refresh brand list after new brand creation (use SWR or simple refetch)

3. Create `src/app/api/conversations/[conversationId]/messages/route.ts` DELETE handler:
...
8. Verify navigation to /[brandId] and sidebar shows new brand
...
- Sidebar updates with new brands
