...
must_haves:
  truths:
    ...
    - "detectGenerationIntent and triggerGenerationFromChat are exported for Phase 6 chat-canvas integration"
  artifacts:
    ...
    - path: "src/lib/ai/chat.ts"
      provides: "Generation intent detection and programmatic generation trigger functions"
      exports: ["detectGenerationIntent", "triggerGenerationFromChat"]
  key_links:
    ...
    - from: "src/lib/ai/chat.ts"
      to: "/api/generate"
      via: "fetch POST for programmatic generation"
...
Purpose: This completes HIST-01, HIST-02 (history viewing/reuse). The chat generation functions are created here as utilities but the actual chat UI wiring (Chat UI -> detectGenerationIntent -> triggerGenerationFromChat -> canvas) is deferred to Phase 6 (Integration & Export), where the chat-canvas connection is built. This avoids coupling the chat sidebar to canvas logic prematurely.

Output: Generation history API + panel component, chat generation detection utilities ready for Phase 6 wiring.
</objective>
...
@.planning/phases/05-ai-generation/05-02-SUMMARY.md
...
<task type="auto">
  <name>Task 2: Create chat generation utilities and integrate history panel into canvas</name>
  <files>
    src/lib/ai/chat.ts
    src/components/canvas/canvas-workspace.tsx
    src/components/canvas/index.ts
...
1. Create or update `src/lib/ai/chat.ts` to add generation detection:
   ...
   - NOTE: These functions will be called from client-side components. The actual wiring into the chat UI (where the chat input calls detectGenerationIntent on submit, and on match calls triggerGenerationFromChat instead of sending a chat message) is DEFERRED to Phase 6 (Integration & Export). Phase 6 will build the chat-canvas sidebar integration, and that's where these functions get wired into the chat UI flow: Chat input -> detectGenerationIntent -> if generation, triggerGenerationFromChat -> add results to canvas. For now, these are standalone utility exports.

2. Update `src/components/canvas/canvas-workspace.tsx`:
   - Import GenerationHistory from './generation-history'
   - Add GenerationHistory to the render, alongside the existing CanvasToolbar and GenerationToolbar (which were added in Plan 05-02):
     ```
     <CanvasToolbar />
     <GenerationToolbar />
     <GenerationHistory />
...
3. Update `src/components/canvas/index.ts`:
   ...
   - Add exports: `export { GenerationToolbar } from './generation-toolbar'` (if not already added by Plan 05-02)
  ...
  <done>
    ...
    - Both functions exported and ready for Phase 6 chat UI wiring (Chat input -> detectGenerationIntent -> triggerGenerationFromChat -> canvas)
...
- triggerGenerationFromChat provides programmatic generation access
- Chat UI wiring is explicitly deferred to Phase 6 (chat-canvas integration)
