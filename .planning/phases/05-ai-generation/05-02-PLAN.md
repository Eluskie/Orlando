...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/components/canvas/generation-toolbar.tsx"
      provides: "Generate button with prompt input popover"
...
<objective>
Build the canvas generation UI: a PlaceholderNode for optimistic loading states, a GenerationToolbar with prompt input, and wire the complete flow - user enters prompt, placeholder nodes appear instantly on canvas, API call fires, placeholders are replaced with real image nodes on completion.

Purpose: This is the core UX for generation (GEN-05, GEN-07). Users need immediate visual feedback when generating, not a blank wait. The optimistic UI pattern with placeholder nodes makes generation feel instant and keeps users engaged during the 2-30 second generation time.

Output: Working generate-from-canvas flow with placeholder -> real image transition, integrated into the existing canvas workspace.
</objective>
...
<task type="auto">
  <name>Task 2: Add GenerationToolbar and wire optimistic generation flow</name>
  <files>
    src/components/canvas/generation-toolbar.tsx
    src/components/canvas/canvas-workspace.tsx
  </files>
...
1. Create `src/components/canvas/generation-toolbar.tsx`:
   - 'use client' directive
   - Import: useState from react, Sparkles/X from lucide-react, useCanvasStore from canvas-store, useGenerationStore from generation-store, useBrandStore from brand-store
   - Export `GenerationToolbar` component
   - State: isOpen (boolean, default false), prompt (string), count (number, default 4), isGenerating (boolean)
   ...
   - On generate click, run the full optimistic flow:
     ...
     b. Create placeholder IDs: `placeholder-${Date.now()}-${i}` for each count
     c. For each placeholder: call addPlaceholder(id, { prompt, brandId: activeBrandId })
     d. For each placeholder: call addNode to canvas with type: 'placeholder', position distributed horizontally (x: 100 + i * 220, y: 100), data: { prompt, status: 'generating' }
     e. Call fetch POST /api/generate with { prompt, brandId: activeBrandId, count }
     f. On success response (response.json()):
     ...
     g. On error: update placeholder statuses to 'error', show console.error
     h. Set isGenerating = false, clear prompt, close panel
...
2. Update `src/components/canvas/canvas-workspace.tsx`:
  ...
  <verify>
    ...
    - Click Generate button: panel opens with prompt input and count selector
  ...
  <done>
    ...
    - Submitting triggers optimistic flow: placeholders appear instantly, API call fires, placeholders replaced with real image nodes
...
5. PlaceholderNode shows spinner and prompt text during generation
6. Multiple variations (1-4) appear as separate nodes on canvas
</verification>
