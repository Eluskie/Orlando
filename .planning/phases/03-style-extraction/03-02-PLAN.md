...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/app/api/chat/route.ts"
      provides: "Image processing in chat flow"
      contains: "type.*image"
  key_links:
...
<objective>
Implement AI-powered style extraction with Zod schema

Purpose: The core of Phase 3 - analyze reference images using Gemini Vision and extract structured style data (colors, mood, typography, visual characteristics) into a validated JSON format.
...
- Updated chat API to detect images and trigger extraction
...
<task type="auto">
  <name>Task 1: Create style extraction schema and function</name>
  <files>
    src/lib/ai/style-extraction.ts
    src/types/brand.ts
    package.json
...
```
  ...
  <done>
    Style extraction schema defined with Zod. extractStyleFromImages uses Gemini 2.5 Flash with Output.object(). mockExtractStyle provides dev fallback. BrandStyle type extended.
  </done>
</task>

<task type="auto">
  <name>Task 2: Integrate style extraction into chat API</name>
  <files>
    src/app/api/chat/route.ts
    src/lib/ai/mock.ts
  </files>
...
```typescript
import { extractStyleFromImages, mockExtractStyle } from '@/lib/ai/style-extraction';
```
...
c) After getting messages from request, check for images and extract style:
```typescript
...
d) Include extraction in system prompt context if available:
```typescript
const systemWithContext = extractedStyle
  ? `${SYSTEM_PROMPT}\n\n[EXTRACTED STYLE]\n${JSON.stringify(extractedStyle, null, 2)}\n\nIncorporate this extracted style into your response.`
  : SYSTEM_PROMPT;
```

e) Pass systemWithContext to streamText and mock response
...
```
  ...
  <done>
    Chat API detects image parts, extracts style (real or mock), includes extraction context in AI response. System prompt updated to guide extraction feedback.
  </done>
</task>
